...
namespace Lab2
{
    public partial class Form1 : Form
    {
        ...
        private void buttonFilter_Click(object sender, EventArgs e)
        {
            ...
            if (maskedTextBoxBirthDate.Text != "  .  .") 
            {
                filter = filter.Where(x => x.BirtDate == Convert.ToDateTime(maskedTextBoxBirthDate.Text));
            }
        ...
        {
            var filter = from person in listP
                         where person.Dad.Equals(listBoxFilter.SelectedItem)
                         select person;
            var filter2 = from person2 in listP
